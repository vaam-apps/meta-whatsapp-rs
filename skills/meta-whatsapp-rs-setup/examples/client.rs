//! Reference code for the `meta-whatsapp-rs-setup` skill: building the `Client` once,
//! and acting as a merchant with `with_token`.
//!
//! meta-whatsapp-rs compiles this file and runs its tests in its own gate
//! (`crates/meta-whatsapp-rs/tests/skills.rs`), so it matches the API of the commit the
//! skill is stamped with.

use std::time::Duration;

use meta_whatsapp_rs::adapters::http::ReqwestTransport;
...
pub fn configured_client(token: AccessToken) -> meta_whatsapp_rs::Result<Client> {

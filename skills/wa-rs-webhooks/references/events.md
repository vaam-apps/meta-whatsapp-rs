...
> Verified against wa-rs 91431ae (2026-09-24): `crates/wa-webhooks/src/event.rs`,
...
`id: MessageId`, `timestamp`, `from: Option<WaId>`, `from_user_id:
Option<UserId>`, `group_id`, `context` (the message it replies to, forwarded
flags, referred product), `referral` (click-to-WhatsApp ads), `errors`, and
`content: MessageContent`:

`Text`, `Image`/`Audio`/`Video`/`Document`/`Sticker` (`MediaContent`: `id`,
`mime_type`, `sha256`, `caption`, `filename`, …), `Location`, `Contacts`,
`Interactive(InteractiveReply::{ButtonReply, ListReply, NfmReply (Flow
response; `.response::<T>()`), CallPermissionReply})`, `Button` (template

//! Reference code for the `wa-rs-templates` skill: template definitions
...
use wa_rs::core::ids::{TemplateId, UploadHandle};
use wa_rs::prelude::*;
use wa_rs::webhooks::fields::TemplateStatusEvent;

/// A utility template with positional placeholders and a URL button.
pub async fn create_order_update(templates: &Templates) -> wa_rs::Result<TemplateCreated> {
...
pub async fn approved(templates: &Templates) -> wa_rs::Result<Vec<TemplateInfo>> {
...
pub async fn reword(templates: &Templates, id: &TemplateId) -> wa_rs::Result<()> {
...
mod tests {
    ...
    fn review_webhooks() {
        ...
        let payload = wa_rs::webhooks::WebhookPayload::from_slice(body.to_string().as_bytes());

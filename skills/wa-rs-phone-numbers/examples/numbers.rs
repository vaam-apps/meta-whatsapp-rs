//! Reference code for the `wa-rs-phone-numbers` skill: registering a
...
use wa_rs::client::business_profile::{ProfileField, ProfileUpdate};
use wa_rs::client::phone_numbers::{
    BotCommand, ConversationalAutomationConfig, SmbSyncType, TwoStepPin,
};
use wa_rs::client::waba::CallbackOverride;
use wa_rs::prelude::*;
...
pub async fn set_up_profile(client: &Client, phone_number_id: PhoneNumberId) -> wa_rs::Result<()> {
...
pub async fn rename(client: &Client, phone_number_id: PhoneNumberId) -> wa_rs::Result<()> {
    let number = client.phone_number(phone_number_id);
...
pub async fn health(client: &Client, phone_number_id: PhoneNumberId) -> wa_rs::Result<()> {

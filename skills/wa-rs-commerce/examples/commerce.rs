//! Reference code for the `wa-rs-commerce` skill: commerce settings,
...
use wa_rs::client::messages::{Messages, ProductSection};
use wa_rs::client::templates::{
    Button, MpmSection, TemplateCategory, TemplateComponent, TemplateDefinition,
};
use wa_rs::core::ids::CatalogId;
use wa_rs::prelude::*;
use wa_rs::webhooks::fields::{MessageContent as Inbound, OrderContent};

/// Show the storefront icon and keep the cart on.
pub async fn open_shop(client: &Client, phone_number_id: PhoneNumberId) -> wa_rs::Result<()> {

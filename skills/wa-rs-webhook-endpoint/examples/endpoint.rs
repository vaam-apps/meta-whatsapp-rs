//! Reference code for the `wa-rs-webhook-endpoint` skill: the endpoint Meta
...
) -> wa_rs::Result<WebhookHandler> {
    if verify_token.trim().is_empty() {
        // A blank token would answer 403 to every verification, at runtime only.
        return Err(wa_rs::core::error::ConfigError::new("WA_VERIFY_TOKEN is blank").into());
    }
...
pub fn routes(handler: WebhookHandler) -> wa_rs::webhooks::axum::Router {
...
pub async fn answer_post<Fut>(
    ...
    signature: Option<&str>, // the `wa_rs::webhooks::SIGNATURE_HEADER` header
...
pub fn status_of(result: &wa_rs::Result<DeliveryReport>) -> u16 {
    match result {
...
mod tests {
    ...
    use wa_rs::webhooks::axum::http::{Request, StatusCode};
    ...
    async fn axum_verification_and_delivery() {
        ...
        let signed = Request::post("/webhooks/whatsapp")
            .header(wa_rs::webhooks::SIGNATURE_HEADER, signature)
            .body(Body::from(body()))
            .unwrap();

//! Reference code for the `meta-whatsapp-rs-media` skill: upload, SHA-256-verified
...
use meta_whatsapp_rs::client::media::{DownloadedMedia, validate_upload};
use meta_whatsapp_rs::client::messages::{Image, OutboundMessage};
use meta_whatsapp_rs::core::ids::{AppId, MediaId, UploadHandle};
...
pub async fn upload_and_send(
    ...
    to: Recipient,
    png: Vec<u8>,
) -> meta_whatsapp_rs::Result<MediaId> {
...
) -> meta_whatsapp_rs::Result<DownloadedMedia> {
...
) -> meta_whatsapp_rs::Result<Vec<u8>> {
    let download = client
...
pub async fn header_handle(
    ...
    app_id: &AppId,
    png: Vec<u8>,
) -> meta_whatsapp_rs::Result<UploadHandle> {
    let media = client.media(phone_number_id);
    media

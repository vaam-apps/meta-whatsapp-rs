//! Reference code for the `wa-rs-embedded-signup` skill: the backend of
...
//! `crates/wa-rs/examples/embedded_signup.rs`. wa-rs compiles this file and
//! runs its tests in its own gate (`crates/wa-rs/tests/skills.rs`).
...
pub async fn start(signup: &Signup, merchant_id: &str) -> wa_rs::Result<serde_json::Value> {
    let state = signup
        .sessions

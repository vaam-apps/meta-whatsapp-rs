//! Reference code for the `wa-rs-errors` skill: deciding what a failed call
...
mod tests {
    ...
    fn the_queue_rule() {
        let refused = |code: i64, status: u16| {
            let mut e = wa_rs::GraphApiError::new(code, "test");
        ...
        let timeout = Error::from(wa_rs::core::error::TransportError::Timeout);
        assert_eq!(after_failed_send(&timeout), Resend::ReconcileFirst);
        let local = Error::from(wa_rs::core::error::ValidationError::new("to", "empty"));

//! Reference code for the `wa-rs-production` skill: secrets loaded once and
...
use wa_rs::prelude::*;
use wa_rs::webhooks::DeliveryReport;
...
/// wa-rs logs through `tracing`: sizes, digests, field names, error kinds,
/// never payload values or secrets. `RUST_LOG=info,wa_client=debug`.
pub fn init_logs() {
...
pub fn client(settings: &Settings) -> wa_rs::Result<Client> {
...
pub fn record_webhook(result: &wa_rs::Result<DeliveryReport>, events: &[WebhookEvent]) {
    match result {

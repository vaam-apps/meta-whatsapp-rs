//! Reference code for the `wa-rs-live-updates` skill: where webhook events
...
) -> impl wa_rs::webhooks::axum::response::IntoResponse {
...
mod tests {
    ...
    fn text_event(number: &str) -> WebhookEvent {
        ...
        wa_rs::webhooks::WebhookPayload::from_slice(body.to_string().as_bytes())
            .unwrap()
            .into_events()
            .remove(0)
    }

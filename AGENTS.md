...
| `just test-live-clean` | drops the `wa_test_*` databases and `wa_test_*` / `wa_server_test_*` schemas live runs left on the test Postgres (killed runs; a panicking adapter test drops its own); not during a live run |

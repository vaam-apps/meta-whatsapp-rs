...
- URLs become procedures (`POST /[v1/]$procs/<name>`: no `GET`, no path
  parameters).
- Two error shapes until D16 lands upstream; until then our outer layer
  renders §5 and CrateStack's own shape never reaches a caller.
...
- chrono and a system principal enter the service's dependency graph,
  and `ring` is linked beside aws-lc-rs (D20).
...
and which stand under the delegation. D6, D8–D12, D19, D20 and D26–D29
are the coordinator's decisions of 2026-09-26 under the delegation, as
are the changes to D15 (widened), D16 (the gate) and D18 (revised) that
day. D5 is settled as "support both modes, chosen per deployment";
which mode a production deployment runs follows the platform's
...
| D20 | CrateStack's dependencies | allow BlueOak-1.0.0 (`minicbor`) in `deny.toml`; accept `ring` beside `aws-lc-rs` (explicit TLS provider at start); accept its `sqlx =0.9.0` pin / change them upstream first | **Coordinator's decision 2026-09-26 under the owner's delegation, swappable:** (a) a `deny.toml` licence exception for BlueOak-1.0.0 scoped to `minicbor` and `minicbor-serde`, because CrateStack's own `deny.toml` allows that licence and `minicbor` is a non-optional dependency of the default the owner asked for. Licences are the owner's: the exception is taken on that reading and enters `deny.toml` only at [roadmap.md](../roadmap.md) S5, where the owner can refuse it; to swap, the CrateStack crates move to a separate workspace and the main one needs no exception. (b) aws-lc-rs installed as rustls's process default at start (`ring` is linked beside it until upstream U2). (c) CrateStack's `sqlx =0.9.0` pin accepted, with an upstream PR relaxing it (U3) | S5 |
...
| D26 | The service's architecture | one crate / a framework-free core with ports, API adapters and backend bundles | **Coordinator's decision 2026-09-26 under the owner's delegation, swappable: the modular service ([§8](#8-modular-architecture-and-client-sdks)).** Defaults: `api-cratestack` and `store-cratestack` (the hybrid). Kept built and conformance-tested: `api-axum` (today's v1 and its OpenAPI document) and `store-postgres` (sqlx); `store-memory` for development; later `store-mongodb`, after the library's MongoDB adapters, with an `Outbox` spike first. To swap, Cargo features choose which adapters are compiled in and configuration picks one of each; a new backend implements the core ports and passes `meta_whatsapp_server_core::conformance` | [roadmap.md](../roadmap.md) S1–S12 |

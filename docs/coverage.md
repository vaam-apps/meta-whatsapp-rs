...
| 26 | CMS inbox: webhook events → `ConversationStore` (statuses scoped to their business number, U+0000 stored as U+FFFD, coexistence echoes and history: row 21), 24h-window-checked replies with the merchant's token; runnable examples in `crates/wa-rs/examples/` (`cms_inbox` and `embedded_signup` servers with per-tenant bearer auth, `otp_login`, `send_message`, `invoice_document`) | CMS | `wa_rs::inbox` | — | **done**; the window ignores calls, which reopen it on Meta's side (`OPEN_QUESTIONS.md` #32) |

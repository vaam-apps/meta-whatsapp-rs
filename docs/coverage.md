...
| 21 | Coexistence (WhatsApp Business app users): onboarding feature type, contacts/history sync, `history` / `smb_app_state_sync` / `smb_message_echoes` webhooks, echoes and synced history recorded by the inbox (direction per the docs, idempotent by message id, chunks in any order, malformed items skipped) | CMS | `embedded_signup`, `phone_numbers`, `wa-webhooks`, `wa_rs::inbox` | `embedded-signup/onboarding-business-app-users`, `webhooks/reference/history`, `webhooks/reference/smb_message_echoes` | **done**; synced inbound messages count towards the local window and unread count, and media contents are not merged into recorded placeholders (both need a port change: `OPEN_QUESTIONS.md` #35); the contacts sync (`smb_app_state_sync`) is not stored |
...
| 26 | CMS inbox: webhook events → `ConversationStore` (statuses scoped to their business number, U+0000 stored as U+FFFD, coexistence echoes and history: row 21), 24h-window-checked replies with the merchant's token; runnable examples in `crates/wa-rs/examples/` (`cms_inbox` and `embedded_signup` servers with per-tenant bearer auth, `otp_login`, `send_message`, `invoice_document`) | CMS | `wa_rs::inbox` | — | **done**; the window ignores calls, which reopen it on Meta's side (`OPEN_QUESTIONS.md` #32), and counts synced history (#35) |

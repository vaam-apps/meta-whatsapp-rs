...
```text
POST ─► body limit (3 MiB) ─► X-Hub-Signature-256 over the raw bytes (any of N app secrets)
...
let app = axum::Router::new().nest("/webhooks/whatsapp", wa_rs::webhooks::router(Arc::new(handler)));
```
...
| missing, malformed or wrong signature | `401` | retries |
...
deserializes from the query string (it uses Meta's `hub.*` names).
...
// POST: status_of(&handler.deliver(signature_header, &raw_body).await)
```
...
| `RedisKvStore` | production; eviction policy `noeviction` or `volatile-*` |

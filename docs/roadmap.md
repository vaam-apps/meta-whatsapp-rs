...
- Each item is one pull request and names its crate, what it comes
...
- [ ] **L11d. The coexistence sync inside onboarding**
  (`client::embedded_signup`; row 124; OPEN_QUESTIONS #7): a step right
  after a coexistence onboarding, on by default, that an option turns
  off.
...
- [ ] **M4. Packaging** (rows 115, 118): the Docker image (D9: public
  ...
  smoke test, the TypeScript client published from S15 (D11: npm;
  publishing is the owner's), the documents route.
  - **After:** S15, M2c.
...
- The parity-completion report goes to the owner, with everything
  decided on the way that is not an owner touchpoint.

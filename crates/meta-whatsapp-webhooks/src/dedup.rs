...
//! handler answers non-`200` ([`meta_whatsapp_core::error::WebhookError::ClaimInFlight`]) and Meta tries again later,
...
/// should be fast; see `meta_whatsapp_core::sink`), and bounds how long an event
...
impl DedupGuard {
    ...
    pub async fn claim(&self, event: &WebhookEvent) -> meta_whatsapp_core::Result<Claim> {
    ...
    pub async fn complete(&self, ticket: &ClaimTicket) -> meta_whatsapp_core::Result<bool> {
        let swapped = self
            .kv
            .compare_and_swap(
                &ticket.key,
    ...
    pub async fn release(&self, ticket: &ClaimTicket) -> meta_whatsapp_core::Result<bool> {
        let removed = self
            .kv
    ...
    pub(crate) async fn claim_key(&self, dedup_key: &str) -> meta_whatsapp_core::Result<Claim> {

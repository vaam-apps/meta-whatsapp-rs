...
//! | Module | Crate | What |
//! | --- | --- | --- |
//! | [`mod@client`] | `meta-whatsapp-client` | Graph API client, one module per endpoint family |
//! | [`webhooks`] | `meta-whatsapp-webhooks` | signature and verify-token checks, typed events, dedup, axum router + SSE |
...
//!
//! The examples live in `crates/meta-whatsapp-rs/examples/`; each file's header lists
...
//! # fn demo(
//! #     app_secret: meta_whatsapp_rs::core::secret::AppSecret,
//! #     verify_token: meta_whatsapp_rs::core::secret::VerifyToken,
//! #     kv: std::sync::Arc<dyn meta_whatsapp_rs::core::store::KvStore>,
//! #     conversations: std::sync::Arc<dyn meta_whatsapp_rs::core::store::ConversationStore>,
//! # ) -> meta_whatsapp_rs::Result<meta_whatsapp_rs::webhooks::axum::Router> {
//! use std::sync::Arc;
...
//! let router = meta_whatsapp_rs::webhooks::axum::Router::new().nest("/webhook", meta_whatsapp_rs::webhooks::router(Arc::new(handler)));
...
//! ```no_run
//! # async fn demo(
//! #     client: meta_whatsapp_rs::Client,
//! #     vault: meta_whatsapp_rs::client::embedded_signup::TokenVault,
//! #     conversations: std::sync::Arc<dyn meta_whatsapp_rs::core::store::ConversationStore>,
//! #     number: meta_whatsapp_rs::core::ids::PhoneNumberId,
...
//! ```no_run
//! # #[cfg(feature = "memory")]
//! # async fn demo(client: meta_whatsapp_rs::Client) -> anyhow::Result<()> {
//! use std::sync::Arc;
//! use meta_whatsapp_rs::adapters::store::MemoryKvStore;
//! use meta_whatsapp_rs::client::authentication::{
...
//! | --- | --- | --- |
// `client` is both a module (the `meta-whatsapp-client` re-export) and a function (the
...
#[cfg(feature = "typst")]
pub use meta_whatsapp_typst as typst;
pub use meta_whatsapp_webhooks as webhooks;
...
///
/// ```no_run
/// # fn demo() -> meta_whatsapp_rs::Result<()> {
/// let client = meta_whatsapp_rs::client_builder()?.build()?;
/// # let _ = client; Ok(()) }
/// ```
///
...
pub fn client_builder() -> Result<ClientBuilder> {
    let transport = meta_whatsapp_adapters::http::ReqwestTransport::new()?;
...
/// ```
#[cfg(feature = "reqwest")]
pub fn client(token: impl Into<meta_whatsapp_core::secret::AccessToken>) -> Result<Client> {
    client_builder()?.access_token(token).build()
}

#[cfg(all(test, feature = "reqwest"))]
mod tests {
    use meta_whatsapp_core::config::ApiVersion;
    ...
    fn client_is_production_graph_with_the_token() {
        ...
        assert_eq!(
            client
                .token()
                .map(meta_whatsapp_core::secret::AccessToken::expose_secret),

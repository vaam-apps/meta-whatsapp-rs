...
//! `meta-whatsapp-rs-embedded-signup` skill's `examples/solution_partner.rs` and the
...
use meta_whatsapp_rs::webhooks::axum::response::{Html, IntoResponse, Response};
use meta_whatsapp_rs::webhooks::axum::routing::{get, post};
use meta_whatsapp_rs::webhooks::axum::{self, Extension, Json, Router};
...
async fn kv_store() -> anyhow::Result<Arc<dyn KvStore>> {
    match std::env::var("DATABASE_URL") {
        ...
        Ok(url) => {
            // sqlx as meta-whatsapp-rs re-exports it: the `PgPool` the stores take.

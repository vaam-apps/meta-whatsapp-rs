...
//! `WA_RS_TEST_REDIS_URL` is set; `WA_RS_REQUIRE_LIVE=1` (as in
...
use wa_adapters::store::{RedisKvStore, conformance};
use wa_core::store::{Expiry, KvStore, StoreKey};

fn client() -> Option<redis::Client> {
    let url = common::service_url("WA_RS_TEST_REDIS_URL")?;
    Some(redis::Client::open(url).expect("valid WA_RS_TEST_REDIS_URL"))
}

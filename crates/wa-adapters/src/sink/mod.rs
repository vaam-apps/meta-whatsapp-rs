//! [`EventSink`](wa_core::sink::EventSink) adapters and event streams
//! (feature `sinks`). All of them are generic over the event type `E`, so
//! they work for `wa_webhooks::WebhookEvent` and for your own events alike.
//!
//! | Sink | Delivers to | When it fails |
//! | --- | --- | --- |
//! | [`ChannelSink`] | a worker, through a bounded tokio `mpsc` channel | receiver dropped ([`Closed`](wa_core::error::SinkError::Closed)); full in [`ChannelMode::TryOrFail`] ([`Full`](wa_core::error::SinkError::Full)) |

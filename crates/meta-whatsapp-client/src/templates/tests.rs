...
async fn list_stream_follows_cursors_and_limit_zero_is_refused() {
    ...
    assert!(
        matches!(items.as_slice(), [Err(meta_whatsapp_core::Error::Validation(v))] if v.field == "after"),
        "{items:?}"
    );
...
async fn invalid_definitions_never_reach_meta() {
    ...
    assert!(matches!(err, meta_whatsapp_core::Error::Validation(ref v) if v.field == "name"));
...
async fn ids_cannot_escape_their_path_segment() {
    ...
    let e = client(&t)
        ...
        .await
        .unwrap_err();
    assert!(matches!(e, meta_whatsapp_core::Error::Validation(_)), "{e}");
    let e = client(&t)
        .authentication("..")

...
use meta_whatsapp_core::transport::HttpTransport;
...
impl ClientBuilder {
    /// HTTP transport. Required. `meta_whatsapp_adapters::http::ReqwestTransport` is the
    /// stock one.
    #[must_use]
    ...
    pub fn build(self) -> Result<Client> {
        let transport = self.transport.ok_or_else(|| {
            Error::from(ConfigError::new(
                "a transport is required (e.g. meta_whatsapp_adapters::http::ReqwestTransport)",
            ))
        })?;

...
fn endpoint_errors_convert_into_the_root_error() {
    ...
    // each to an HTTP status); `?` must still lift them into meta_whatsapp_core::Error.
    fn signature(body: &[u8]) -> meta_whatsapp_core::Result<()> {
        verify_request_signature(body, None, &[])?;
        Ok(())
    }
    fn decrypt(key: &FlowEndpointKey) -> meta_whatsapp_core::Result<FlowRequest> {
    ...
    assert!(matches!(
        decrypt(&key),
        Err(meta_whatsapp_core::Error::Crypto(CryptoError::Decrypt))
    ));
}

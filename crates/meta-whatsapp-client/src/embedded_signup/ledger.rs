...
pub struct StoredCredit {
    ...
    ///
    /// Not an audit log on its own: a meta-whatsapp-rs revision older than this field
...
pub struct ClearedShare {
    ...
    /// token and is never deleted by meta-whatsapp-rs; meta-whatsapp-rs never logs it, and
...
mod tests {
    ...
    async fn rotation_reaches_every_ledger_record() {
        ...
        old.store(&super::super::vault::StoredBusinessToken::new(
            "CORRUPT",
            meta_whatsapp_core::secret::AccessToken::new("EAAB"),
        ))
        .await
        .unwrap();

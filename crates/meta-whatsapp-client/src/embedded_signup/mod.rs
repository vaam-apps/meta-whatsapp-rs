...
//! ```no_run
//! # async fn demo(
//! #     client: meta_whatsapp_client::Client,
//! #     kv: std::sync::Arc<dyn meta_whatsapp_core::store::KvStore>,
//! #     merchant_id: &str,
//! #     posted_state: &str,
//! #     posted_code: &str,
//! #     posted_event: &str,
...
//!
//! use meta_whatsapp_client::AppCredentials;
//! use meta_whatsapp_client::embedded_signup::{
//!     EmbeddedSignupEvent, LaunchOptions, OnboardingRequest, SignupCode, SignupSessions,
//!     SignupState, TokenVault, VaultKey, VaultKeys,
//! };
//! use meta_whatsapp_client::phone_numbers::TwoStepPin;
//!
...
//!
//! [`EmbeddedSignup::onboard`] fails with [`meta_whatsapp_core::Error::Step`] naming the
...
//! [`meta_whatsapp_core::ErrorKind::Registration`] and is never retried automatically.
//!
...
//!   with a [`ValidationError`](meta_whatsapp_core::error::ValidationError) before the
...
//!   ([`CreditError::ApprovalRequired`](meta_whatsapp_core::error::CreditError::ApprovalRequired),
...
//!   may onboard a WABA is your policy; meta-whatsapp-rs decides none
//!   (`OPEN_QUESTIONS.md` #6).
...
//!   step, is [`CreditError::Busy`](meta_whatsapp_core::error::CreditError::Busy);
...
//!   [`CreditError::Reconcile`](meta_whatsapp_core::error::CreditError::Reconcile)
...
//!   ([`CreditError::OwnerUnknown`](meta_whatsapp_core::error::CreditError::OwnerUnknown)).
...
//!   refuses ([`CreditError::Revoked`](meta_whatsapp_core::error::CreditError::Revoked);
//!   a record whose `request_status` Meta does not document is
//!   [`CreditError::StatusUnknown`](meta_whatsapp_core::error::CreditError::StatusUnknown))
...
//!   [`CreditError::RevocationIncomplete`](meta_whatsapp_core::error::CreditError::RevocationIncomplete),
//!   with the report.
//!

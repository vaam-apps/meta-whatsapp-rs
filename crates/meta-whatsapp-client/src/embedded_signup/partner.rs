...
impl EmbeddedSignup {
    ...
    /// of your line for the business). meta-whatsapp-rs has no call that edits the
    /// sealed record otherwise, and a hand edit of the store makes it
    /// unreadable (then treated as a share by `offboard`): ask the meta-whatsapp-rs
    ...
    /// the WABA's credit record, which outlives the token (meta-whatsapp-rs never logs
...
mod tests {
    ...
    impl meta_whatsapp_core::transport::HttpTransport for Hooked {
        async fn send(
            &self,
            request: meta_whatsapp_core::transport::HttpRequest,
        ) -> Result<meta_whatsapp_core::transport::HttpResponse, TransportError> {
            let hook = {
    ...
    impl TestKv {
        ...
        ) -> Result<(), meta_whatsapp_core::error::StorageError> {
    ...
    #[async_trait::async_trait]
    impl meta_whatsapp_core::store::KvStore for TestKv {
        async fn get(
            &self,
            key: &meta_whatsapp_core::store::StoreKey,
        ...
        async fn put(
            ...
            key: &meta_whatsapp_core::store::StoreKey,
            value: Vec<u8>,
            expiry: meta_whatsapp_core::store::Expiry,
        ...
        async fn compare_and_swap(
                ...
                && let Some(current) = self.inner.get(key).await?
            {
                ...
                self.inner
                    .put(key, current.value, meta_whatsapp_core::store::Expiry::Keep)
                    .await?;
            }
            self.inner
    ...
    fn lease_key() -> meta_whatsapp_core::store::StoreKey {
        meta_whatsapp_core::store::StoreKey::new(
    ...
    async fn revocation_survives_an_unreadable_token_record() {
        ...
        let bytes = other
            .kv()
            .get(&meta_whatsapp_core::store::StoreKey::new(
                super::super::vault::TOKEN_NAMESPACE,
                format!("waba/{WABA}"),
            ))
            .await
    ...
    fn credit_store_key() -> meta_whatsapp_core::store::StoreKey {
        meta_whatsapp_core::store::StoreKey::new(
            super::super::vault::TOKEN_NAMESPACE,
            format!("credit/{WABA}"),
        )

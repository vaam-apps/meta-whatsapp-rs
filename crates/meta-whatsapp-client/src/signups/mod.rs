...
//!
//! Error codes: `2494164` not found ([`meta_whatsapp_core::ErrorKind::NotFound`]),
//! `2494165` API not enabled for the WABA
//! ([`meta_whatsapp_core::ErrorKind::FeatureNotAvailable`]); `2494166` unknown
...
//! [`meta_whatsapp_core::ErrorKind::InvalidParameter`]; read
//! [`meta_whatsapp_core::GraphApiError::code`] to tell them apart).
//!
//! # Terms of Service
//!
...
mod tests {
    ...
    fn field(err: &meta_whatsapp_core::Error) -> String {
        match err {
            meta_whatsapp_core::Error::Validation(v) => v.field.clone(),
    ...
    fn deep_link_refuses_numbers_that_are_not_strict_e164() {
        ...
        for bad in [
            ...
            assert!(
                matches!(&err, meta_whatsapp_core::Error::Validation(v) if v.field == "phone_number"),
                "{bad:?}: {err}"
            );
        }
    }
    ...
    async fn terms_of_service_errors_are_classified() {
        ...
        let s = client(&t).signups("W");
        for (code, kind) in [
            (2494168, meta_whatsapp_core::ErrorKind::InvalidParameter),
            (2494176, meta_whatsapp_core::ErrorKind::InvalidParameter),
            (2494177, meta_whatsapp_core::ErrorKind::InvalidParameter),
            (2494165, meta_whatsapp_core::ErrorKind::FeatureNotAvailable),
        ] {
            t.push_json(
                400,
        ...
        assert_eq!(err.kind(), meta_whatsapp_core::ErrorKind::NotFound);

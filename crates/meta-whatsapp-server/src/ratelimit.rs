...
//! is `WA_SERVER_RATE_<CLASS>` and `…_BURST` (see [`crate::config`]), the
//! same for every tenant. Per-tenant overrides by the admin API are not
//! implemented.
...
impl RouteClass {
    ...
    /// The class of a route needing `scope`, called with `method`.
    pub fn of(scope: Scope, method: &Method) -> Self {

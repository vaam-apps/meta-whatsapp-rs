...
- **`ErrorKind::as_str()` and `ErrorKind::ALL`**: every error kind has a
  ...
  field or a metrics label, and `ErrorKind::ALL` lists every kind once
  (`Unknown` last; do not rely on positions), so code that must decide something per kind can
  ...
  each one, and the build fails if a kind declared before `Unknown` (which
  stays the last variant) is missing from the list.
